...
\item \textbf{Conditional perplexity reaches \(1\) only for deterministic mappings.} English \(\to\) Englio can be deterministic after pronoun collapse, but Englio \(\to\) English remains ambiguous.
...
\]
\item The embedding layer is mostly a learned lookup table; the nonlinear hidden layers do the contextual feature combination.
\end{itemize}
...
\textbf{RNN emulating a 2-token feed-forward LM:}
\[
W_{hh}=
\begin{bmatrix}
0 & 0 \\
...
h_t=[Wx_{t-1};\,Wx_{t-2}],
\]
so the RNN can reproduce a 2-token feed-forward LM.
\begin{symdefs}
\sym{\(W_{hh}\)}{hidden-to-hidden transition matrix of the RNN}
\sym{\(W_{xh}\)}{input-to-hidden matrix of the RNN}
\sym{\(W\)}{embedding matrix from the original 2-token feed-forward LM}
\sym{\(I_d\)}{\(d \times d\) identity matrix}
\sym{\(0\)}{zero block of the appropriate shape}
\sym{\(d\)}{embedding dimension}
\sym{\(f(z)=z\)}{identity activation function}
