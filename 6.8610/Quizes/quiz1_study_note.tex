...
\item Main course arc:
\[
\begin{aligned}
\text{LMs}
&\rightarrow \text{log-linear / embeddings}
\rightarrow \text{RNNs / attention} \\
&\rightarrow \text{Transformers}
\rightarrow \text{evaluation}
\rightarrow \text{pretraining}
...
\item High dot products reflect co-occurrence structure and similar contexts.
\end{itemize}
...
\]
\begin{symdefs}
\sym{Interpretation}{repeated Jacobian products across time can make gradients vanish or explode}
\end{symdefs}
...
\end{itemize}
\item Self-attention is permutation-invariant without positional information.
\item Transformer encoder = self-attention \(+\) residual/norm \(+\) FFNN.
\item Transformer decoder = masked self-attention; encoder-decoder models also add cross-attention.
...
\textbf{Bilingual Evaluation Understudy (BLEU):}
\[
...
\exp\left(\frac{1}{N}\sum_{n=1}^{N}\log \operatorname{Precision}_n\right).
\]
\begin{symdefs}
\sym{\(\operatorname{BP}\)}{brevity penalty}
\sym{\(N\)}{largest \(n\)-gram order used in BLEU}
\end{symdefs}

\textbf{Recall-Oriented Understudy for Gisting Evaluation (ROUGE)-2:}
\[
...
\textbf{BERTScore recall-style version:}
\[
\operatorname{BERTScore}_{\text{recall}}
=
\frac{1}{|r|}\sum_i \max_j \cos(r_i, p_j).
\]
\begin{symdefs}
\sym{\(r_i\)}{reference-token embeddings}
\sym{\(p_j\)}{prediction-token embeddings}
\sym{\(|r|\)}{number of reference tokens}
\end{symdefs}
...
\textbf{Importance weighting for data mixing:}
\[
w(x)=\frac{\hat{p}_{\mathrm{FT}}(x)}{\hat{p}_{\mathrm{PT}}(x)}.
\]
\begin{symdefs}
\sym{\(w(x)\)}{weight that upweights pretraining examples resembling the downstream domain}
\end{symdefs}

\textbf{Adversarial reweighting template:}
\[
\min_\theta \max_{q \in \Delta^K}
\sum_{k=1}^{K} q_k\big(\mathcal{L}_k(\theta)-\mathcal{L}_k(\theta_{\mathrm{ref}})\big).
\]
\begin{symdefs}
\sym{\(\Delta^K\)}{simplex over \(K\) data subpopulations}
\sym{\(q_k\)}{weight on subpopulation \(k\)}
\sym{\(\theta_{\mathrm{ref}}\)}{reference model}
\end{symdefs}
...
\item Chinchilla refinement: compute-optimal training uses a better parameter/data balance.
...
\item Loss and perplexity remain useful but are increasingly weak proxies for reasoning, safety, and tool use.
\item Progress now depends on more than pretraining scale: post-training, tool use, inference-time reasoning, and better inductive biases matter.
\end{itemize}

\section*{High-Value Contrasts}
\begin{itemize}
\item \textbf{Word embeddings vs contextualized embeddings}: word2vec gives one vector per word type; RNNs/Transformers give token-specific vectors.
\item \textbf{Masked vs autoregressive}: masked objectives are good for representation learning; autoregressive objectives define sequence probabilities and generation.
\item \textbf{CBOW/Skip-gram vs LM}: embedding learners are not proper sequence models.
\item \textbf{RNN attention vs Transformer self-attention}: seq2seq attention reweights encoder states for a decoder; Transformer self-attention lets every token attend to every token in parallel.
\item \textbf{Self-attention vs cross-attention}: self-attention stays within one sequence; cross-attention uses decoder queries with encoder keys/values.
\item \textbf{BERT vs GPT}: BERT is encoder-only and mostly for representation learning; GPT is decoder-only and autoregressive.
\item \textbf{Pretraining vs fine-tuning}: pretraining builds broad capabilities; fine-tuning specializes or steers them.
\item \textbf{Perplexity vs real performance}: useful intrinsic metric, but not sufficient for truthfulness, quality, reasoning, or product value.
\end{itemize}
