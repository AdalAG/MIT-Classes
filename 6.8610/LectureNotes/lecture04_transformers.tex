...
\begin{itemize}
    ...
    \item We also want a model that is highly parallelizable, unlike recurrent neural networks (RNNs) whose hidden states must be computed sequentially.
    \item This leads to self-attention, Transformer encoders, Transformer decoders, and practical pretrained systems such as Bidirectional Encoder Representations from Transformers (BERT) and Generative Pre-trained Transformer (GPT) variants such as GPT-2.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Three ways to attend:}
    \begin{itemize}
        \item \textbf{Self-attention:} operate on one sequence and decide how much each token should attend to the other tokens in that same sequence.
        \item \textbf{Masked self-attention:} still operate on one sequence, but forbid attention to future positions so the model remains autoregressive.
        \item \textbf{Cross-attention:} use one sequence to attend to another, as in an encoder-decoder model.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Supervised learning:} train on labeled inputs and outputs for a specific task.
        \item \textbf{Unsupervised / self-supervised learning:} train on unlabeled text by deriving targets from the data itself, such as predicting masked or future tokens.
        \item \textbf{Semi-supervised learning:} combine a small amount of labeled data with a larger amount of unlabeled data.
        \item \textbf{Multi-task learning:} train a shared model on multiple tasks.
        \item \textbf{Transfer learning:} learn useful capabilities on one task or domain and reuse them on another.
        \item \textbf{Pretraining:} the first stage of transfer learning, usually on a broad unlabeled corpus before downstream adaptation.
    \end{itemize}
\end{itemize}
...
\textbf{Multi-head attention (MHA).} For head \(h\),
\[
...
\textbf{Feed-forward neural network (FFNN) sublayer.} After attention, each token representation is passed through a position-wise feed-forward network:
\[
...
\textbf{Positional encodings (PE).} The original Transformer adds a positional vector \(p_i\) to each token embedding:
\[
x_i' = x_i + p_i.
\]
...
\textbf{Cross-attention.} In an encoder-decoder Transformer, decoder states query the encoder outputs:
\[
Q^{dec} = H^{dec} W_Q, \qquad K^{enc} = H^{enc} W_K, \qquad V^{enc} = H^{enc} W_V,
\]
\[
\operatorname{CrossAttention}(Q^{dec}, K^{enc}, V^{enc})
=
\operatorname{softmax}\!\left(\frac{Q^{dec}(K^{enc})^\top}{\sqrt{d_k}}\right)V^{enc}.
\]
The lecture explicitly distinguished this from decoder self-attention: in cross-attention, the queries come from the current decoder layer, while the keys and values come from the encoder outputs.
...
\begin{itemize}
    ...
    \item Treat the setup as self-supervised pretraining: the corpus is unlabeled, and the prediction targets are created from the text itself.
...
\begin{itemize}
    ...
    \item Treat this as supervised transfer learning: start from pretrained weights, then adapt on a smaller labeled task dataset.
...
\begin{itemize}
    ...
    \item \textbf{What Lecture 3 called ``attention'' is specifically cross-attention in Transformer language.} The professor stressed that once both self-attention and cross-attention exist, the generic word ``attention'' becomes ambiguous.
    \item \textbf{Labels are scarce, so pretraining is the practical answer.} The motivation for BERT fine-tuning was not just elegance; it was that self-supervised pretraining on unlabeled text makes later supervised learning feasible when task labels are limited.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Sentence-pair classification with BERT:} the classification token \texttt{[CLS]} can be used as a summary representation for tasks with one label per input sequence or per sentence pair.
    ...
    \item \textbf{Three ways to attend:} self-attention operates within one sequence, masked self-attention does the same with future positions blocked, and cross-attention lets a decoder attend to a separate encoder sequence.
\end{itemize}
...
\begin{itemize}
    ...
    \item Types of learning discussed in this lecture include supervised, unsupervised/self-supervised, semi-supervised, multi-task, and transfer learning; pretraining is the first stage of transfer learning.
    ...
    \item BERT pretraining is self-supervised on unlabeled text; BERT fine-tuning is supervised transfer learning on a labeled downstream task.
    ...
    \item A Transformer decoder uses \textbf{masked self-attention}; an encoder-decoder Transformer also uses \textbf{cross-attention}, where decoder queries attend to encoder keys and values.
