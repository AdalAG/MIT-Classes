...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{NLP historically consisted of many specialized tasks.} These include syntax-focused tasks (morphology, part-of-speech tagging, parsing), discourse tasks (summarization, coreference), and semantic tasks (sentiment, NER, QA, machine translation, language modeling).
...
\]
This left-to-right factorization is the dominant paradigm in modern large language models.
...
\]
for right-to-left generation, or via latent-variable models such as HMM-style models or diffusion-based formulations.
...
\textbf{Perplexity.} A standard intrinsic evaluation metric for language models is
\[
...
\begin{itemize}
    ...
    \item Choose a model family \(f_\theta\): rule-based, CRF, HMM, graphical model, neural network, or another structured predictor.
...
\begin{itemize}
    ...
    \item The shift from many specialized models to unified \textbf{language modeling} motivates why later lectures focus on log-linear models, RNNs, sequence-to-sequence models, attention, Transformers, and modern LLM training.
