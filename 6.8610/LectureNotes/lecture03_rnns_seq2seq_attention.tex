...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Masked vs.\ autoregressive feed-forward modeling:} with a fixed window, a feed-forward network can either predict a missing token from surrounding context or predict the next token from only the left context. The autoregressive version is the one that directly defines a left-to-right sequence model.
...
\textbf{Feed-forward masked vs.\ autoregressive objectives.} With a fixed-size window, one can train the same feed-forward architecture in two different ways:
\begin{itemize}
    \item \textbf{Masked language modeling:} hide a token and predict it from both left and right context,
    \[
    p_\theta(x_t \mid x_{t-k:t-1}, x_{t+1:t+k}).
    \]
    \item \textbf{Autoregressive language modeling:} predict the next token from only the previous context,
    \[
    p_\theta(x_t \mid x_{t-k:t-1}).
    \]
\end{itemize}
The lecture explicitly contrasted these two views: a masked objective can learn useful contextual representations, but only the autoregressive formulation directly supports a normalized left-to-right probability for the whole sequence \(p(x_{1:T})\).
...
\textbf{Popular attention scoring functions.} Common choices include:
\begin{itemize}
    \item \textbf{Additive / concat scoring:}
    \[
    e_{t,i} = v_a^\top \tanh\!\left(W_a [h^{dec}_t; h^{enc}_i]\right).
    \]
    \item \textbf{Bilinear / general scoring:}
    \[
    e_{t,i} = (h^{dec}_t)^\top W_a h^{enc}_i.
    \]
    \item \textbf{Dot-product scoring:}
    \[
    e_{t,i} = (h^{dec}_t)^\top h^{enc}_i.
    \]
\end{itemize}
The transcript emphasized the tradeoff between flexibility and efficiency: more expressive learned scorers are possible, but dot-product scoring became especially attractive because it is simple, fast, and works well at scale.
...
\textbf{1. Feed-forward masked and autoregressive prediction}
\begin{itemize}
    \item Choose a fixed-size context window.
    \item For a masked objective, remove one token and predict it from its surrounding context.
    \item For an autoregressive objective, keep only the left context and predict the next token.
    \item Convert the visible context into embeddings or a fixed feature vector.
    \item Pass it through one or more feed-forward layers.
    \item Predict the target token with a softmax over the vocabulary.
...
\begin{itemize}
    ...
    \item \textbf{The exact attention scorer is a design choice.} The professor noted that one can use an MLP-style scorer, a dot product, or a linear transformation, but speed matters because the score is computed many times; in practice, simple dot-product forms often win.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Feed-forward masked vs.\ autoregressive modeling:} masked prediction can learn contextual representations from both sides of a token, while autoregressive prediction models the next token from the left context and directly supports sequence probabilities.
    ...
    \item \textbf{Popular attention scoring functions:} additive/concat, bilinear/general, and dot product.
